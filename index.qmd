...
1.  Motivation: Why do I care about functions?
2.  Function Fundamentals
3.  Functions in Practice

## {background-image="images/prior_art_wide.png" background-size="contain"}
...
-   **Copy** text into body
-   Come up with some good **names** (consider verbs?)
...
## {background-image="images/roger-user-developer-spectrum.png" background-size="cover"}

## {background-image="images/hadley-practice-programmer.png" background-size="cover"}
...
https://blog.revolutionanalytics.com/2014/01/john-chambers-recounts-the-history-of-s-and-r.html

Taken from [tapply to Tidyverse, by Roger Peng]()
...
- Getting an error when writing can feel like a punishment
- But hanging out with errors is like, the normal state
- When learning functions, you should be learning debugging along the way
...
my_fun <- function(x, y){
  ...
  # "n" + [RETURN] go to **n**ext line
  
  # "s" + [RETURN] **s**tep into highlighted function
  
  # "f" + [RETURN] **f**inish the rest of the function
}
```
...
- `debug(function_name)`
  - as if "browser" has been turned on at the top of the function. 
  - saves you needing to inject `browser()` into code
  - will run debugging on `function_name()` until end of session
- `undebug(function_name)`
  - Turns off `debug()`
- `debugonce(function_name)`
  - Does `debug(function_name)` exactly one time

# Other debugging tools to explore
...
- Functions helped us get to larger ideas - e.g., repeating this for each sheet, from 2014 - 2023
- Debugging is a crucial step that should be taught alongside using functions
...
- use things like targets! 
- peer review your code with colleagues.
...
- rooted in useful examples
- talk about expression
- DRY is a useful secondary principle to expression
...
- Writing functions is **writing**
- Sprinkle functions into daily use
- Using functions to express ideas
